...
function install_dependencies() {
    ...
    sudo apt-get update
    sudo apt-get install -y curl git python3 python-is-python3 ca-certificates git-lfs build-essential libncurses-dev libssl-dev || print_err "Failed to install system dependencies."
...
EOF
        ...
        repo sync -c -j2 --force-sync --no-clone-bundle --no-tags || print_err "Repo sync failed. Try again later or use a VPN."
    ...
    2)
        ...
        # Copy prebuilt dtb.img to the out folder so boot.img packaging can find it
        mkdir -p $OUT_DIR/target/product/$DEVICE
        cp device/xiaomi/$DEVICE/prebuilt/dtb.img $OUT_DIR/target/product/$DEVICE/dtb.img
